...
mod_histogram_server <- function(id, data_input, selected_box, session){
  moduleServer( id, function(input, output, session_module){
    ...
    output$num_inputs <- renderUI({
      fluidRow(
        column(3, bslib::tooltip(numericInput(ns("lx"), "lx:", 0, min = 0),
                 "Length of unusable wood at stem foot [m], defaults to 0 (X-Holz)",
                 placement = "right"), inline = TRUE ),
        column(3, bslib::tooltip(numericInput(ns("Zsh"), "Zsh:", 0, min = 0),
                                 "Minimum cutting diameter under bark for stem wood [cm], defaults to 0, using parameter Az if estimated length < maximum length (i.e. 20m)",
                                 placement = "right"), inline = TRUE ),
        column(3, bslib::tooltip(numericInput(ns("Zab"), "Zab:", 0, min = 0),
                                 "Minimum cutting diameter under bark for top segment [cm], defaults to 0, i.e. 14cm under bark",
                                 placement = "right"), inline = TRUE ),
        column(3, bslib::tooltip(numericInput(ns("Sokz"), "Sokz:", 1, min = 0, max = 2),
                                 "Assortment calculation, 0 - no assortment, 1 - Mid diameter (Mittenstaerke), 2 - Heilbronner Sortierung. Defaults to 1",
                                 placement = "right"), inline = TRUE ),
        column(3, bslib::tooltip(numericInput(ns("Az"), "Az:", 0, min = 0),
                                 "Minimum cutting diameter over bark [cm]. Defaults to 0. Uses an exponential function given DBH",
                                 placement = "right"), inline = TRUE ),
        column(3, bslib::tooltip(numericInput(ns("fixN"), "fixN:", 0, min = 0),
                                 "Number of fixed length assortments at stem foot, defaults to 0 (no fixed length assortments, irrespective of other fixed parameters)",
                                 placement = "right"), inline = TRUE ),
        column(3, bslib::tooltip(numericInput(ns("fixZ"), "fixZ:", 0, min = 0),
                                 "Mininum diameter under bark for fixed length assortment at stem foot, defaults to 0",
                                 placement = "right"), inline = TRUE ),
        column(3, bslib::tooltip(numericInput(ns("fixL"), "fixL:", 0, min = 0),
                                 "Length of fixed length assortment at stem foot, defaults to 0",
                                 placement = "right"), inline = TRUE ),
        column(3, bslib::tooltip(numericInput(ns("fixA"), "fixA:", 0, min = 0),
                                 "Fixed length assortement add-on in [cm], defaults to 0",
                                 placement = "right"), inline = TRUE ),
        column(3, bslib::tooltip(numericInput(ns("fixR"), "fixR:", 0, min = 0),
                                 "Fixed length assortement add-on in [%], defaults to 0",
                                 placement = "right"), inline = TRUE ),
        column(3, bslib::tooltip(numericInput(ns("Hsh"), "Hsh:", 0, min = 0),
                                 "Maximum height of sawlog quality",
                                 placement = "right"), inline = TRUE ),
